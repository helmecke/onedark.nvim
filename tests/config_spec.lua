...
describe("Using the theme", function()
    ...
    it("it should allow a color palette to be extracted", function()
        local colors = require("onedark.helpers").get_colors(vim.g.colors_name)
        assert.equals("#d55fde", colors.purple)
        assert.equals("onedark", colors.name)
    end)
    ...
    it("it creates the autocmds for inactive windows", function()
        local output = vim.api.nvim_exec("autocmd Onedark", true)

        assert.equals(true, string.find(output, "Onedark") ~= nil)
