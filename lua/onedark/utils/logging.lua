...
function Logger.new(filename, opts)
    ...
    logger._level = opts.level or vim.g.onedark_log_level
    local ok, logpath = pcall(vim.fn.stdpath, "log")
    if not ok then logpath = vim.fn.stdpath("cache") end
    ...
    if log_info and log_info.size > LARGE then
        local warn_msg =
            string.format("OneDark log is large (%d MB): %s", log_info.size / (1000 * 1000), logger._filename)
    ...
    for level, levelnr in pairs(vim.log.levels) do
        logger[level:lower()] = function(...)
            local argc = select("#", ...)
            if levelnr < logger._level then return false end
