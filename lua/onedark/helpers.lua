...
local Color = require("onedark.lib.color")
...
function M.get_preloaded_colors(theme_name)
  return require("onedark.theme").colors(theme_name or config.theme)
end
...
function M.get_colors(theme_name)
  local theme = require("onedark.theme").load(theme_name or config.theme)
  return require("onedark.utils").deep_extend(theme.palette, theme.generated, theme.meta)
end
