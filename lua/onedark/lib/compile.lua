...
local function highlight(name, values, theme, opts)
  ...
  if next(values) == nil then
    return string.format([[set_hl(0, "%s", {})]], name)
  end
...
function M.compile(theme, opts)
  theme = require("onedark.theme").load(theme or config.theme)
  local highlight_groups, custom_groups = require("onedark.highlight").groups(theme)
...
vim.o.background = "%s"]],
  ...
  if config.config.options.terminal_colors then
    local terminal_colours = require("onedark.highlights.terminal").groups(theme)
  ...
  if config.config.options.highlight_inactive_windows or config.config.options.window_unfocused_color then
    table.insert(lines, "\n-- Autocmds\n")
    local autocmds = require("onedark.highlights.autocmd").groups(theme)
